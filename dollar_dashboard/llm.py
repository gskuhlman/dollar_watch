...
def _compact_analysis_payload(payload: dict, max_chars: int = 60000) -> str:
    ...
    if isinstance(data.get("snapshot"),dict):
        keep={"timestamp","market_summary","fred_summary","auction_stress_auto","auction_summary","upcoming_auctions","cftc_usd_downside_pressure","cftc_summary","fx_positioning_squeeze_risk","tic_dedollarization_pressure","tic_meta","tic_transaction_meta","cofer_dedollarization_pressure","cofer_meta","fiscal_flow_stress_auto","fiscal_meta","stablecoin_dollar_support_auto","stablecoin_meta","treasury_buyback_meta","fed_treasury_classification","offshore_usd_funding_meta","official_policy_meta","data_confidence","component_confidence"}
...
def analyze_with_local_llm(
    ...
    system = """You are the red-team macro analyst for dollar_watch V3.2.
...
- Domestic repo/Fed-facility coverage is not full global-dollar-funding coverage. V3.2 may supply a front-futures/spot dislocation proxy; this is NOT cross-currency basis. Explicitly mention the remaining cross-currency-basis/FX-swap gap when Dollar Funding Squeeze coverage is discussed.
...
- V3.2 official_policy_meta contains deterministic primary-source facts. Japan MOF intervention amounts are direct observations, but the monthly total alone does not prove direction or broad U.S. dollar intent. NY Fed U.S. FX-operation reports may establish whether the U.S. intervened directly in the reported quarter.
- V3.2 intervention_history is a quarter-level actor/currency/purpose timeline. A targeted Treasury/ESF intervention (for example Argentina stabilization) is not broad-dollar policy; always state actor, currency, direction/purpose, and broad-USD implication.
- V3.2 may include nyfed_lagged_basis_validation: an official quarterly cross-check of offshore FX-swap funding conditions. It is useful validation context but is too stale to fire a live funding trigger or raise live offshore coverage.
