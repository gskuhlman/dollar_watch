...
def check_source_url(url: str, timeout: int = 8) -> dict:
    ...
    try:
        r = requests.get(url, timeout=timeout, allow_redirects=True, headers={"User-Agent": "dollar_watch/2.1"}, stream=True)
...
def fetch_source_text(url: str, max_chars: int = 30000, timeout: int = 12) -> dict:
    ...
    try:
        r = requests.get(url, timeout=timeout, allow_redirects=True, headers={"User-Agent": "dollar_watch/2.1"})
        r.raise_for_status()
